...
export async function POST(request: Request) {
  try {
    ...
    if (!(file instanceof File) || !file.name) return NextResponse.json({ error: '请选择截图文件' }, { status: 400 });
    if (!ALLOWED.has(file.type)) return NextResponse.json({ error: '只支持 png、jpg、webp、gif 图片' }, { status: 400 });
    if (file.size > MAX_SIZE) return NextResponse.json({ error: '图片不能超过 10MB' }, { status: 400 });
    ...
    return NextResponse.json({ path: uploaded.data.path, name: file.name });
  } catch (error) {
